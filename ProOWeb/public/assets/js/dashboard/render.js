...
export function renderWorkspaceStatus({ status, onMigrate, windowRef = window, documentRef = document }) {
  ...
  const projectTitle = documentRef.getElementById("project-title");
  const stackLine = documentRef.getElementById("stack-line");
  if (projectTitle) {
    projectTitle.textContent = workspace.project.title;
  }
  if (stackLine) {
  ...
  if (migrateButton && migrateFeedback) {
